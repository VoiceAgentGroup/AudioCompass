...
- `--split`: Chooses the data split to evaluate.
    - For most datasets (`alpacaeval`, `commoneval`, `ifeval`, `advbench`, etc), use `test` as the value.
    - For the `sd-qa` subset, you should provide a region code instead of `test`. Available regions are `aus`, `gbr`, `ind_n`, `ind_s`, `irl`, `kenya`, `nga`, `nzl`, `phl`, `usa`, `zaf`.
    - For the `mmsu` subset, you should provide a domain code. Available domains are `law`, `engineering`, `other`, `biology`, `business`, `economics`, `health`, `philosophy`, `psychology`, `history`, `chemistry`, `physics`.
