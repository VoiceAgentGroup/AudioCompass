...
class KimiAPromptManager:
    def __init__(self, model_path: str, kimia_token_offset: int, cache_dir: str):
        audio_tokenizer_path = os.path.join(cache_dir, "glm-4-voice-tokenizer")
        if not os.path.exists(audio_tokenizer_path):
            snapshot_download(
                repo_id="THUDM/glm-4-voice-tokenizer",
                local_dir=cache_dir,
            )
        self.audio_tokenizer = Glm4Tokenizer(audio_tokenizer_path)
